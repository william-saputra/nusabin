...
const ALERT_TEXT = {
  ...
  inorganic_non_recyclable: "Non–Recyclable Chamber Is Full, Collect Your Waste, Now!",
...
function updateFill(binId, percent, volumeLiters) {
  ...
  rect.setAttribute("height", String(newHeight));
  rect.setAttribute("y", String(newY));
  rect.setAttribute("fill", p >= 70 ? "#FF2828" : "#0DC798");
  ...
  // Toggle warning icon overlay for high level
  const warn = item.querySelector('.bucket-warning');
  if (warn) {
...
// Keep track of latest levels to decide alerts
const levelsState = {
  inorganic_non_recyclable: 0,
